...
if (!ENCRYPTION_KEY_B64) {
  throw new Error('API_KEY_ENCRYPTION_KEY_B64 environment variable is required for encryption/decryption.');
}
const ENCRYPTION_KEY = Uint8Array.from(atob(ENCRYPTION_KEY_B64), c => c.charCodeAt(0));
...
export async function decryptApiKey(encrypted: string): Promise<string> {
  ...
  if (!ivB64 || !cipherB64) throw new Error('Invalid encrypted API key format.');
...
serve(async (req) => {
  ...
  try {
    ...
    try {
      ...
      if (!authError && user) {
        ...
        if (!credError && credentials) {
          ...
          if (!encryptedApiKey) {
            throw new Error('Encrypted API key is missing in api_key_encrypted column.');
          }
          ...
          throw new Error(`No user-specific credentials found for user: ${userId}`);
        }
      } else {
    ...
    if (!apiKey || !username) {
      throw new Error('Mspace API credentials not available. Please configure them in user settings.');
    }
      ...
      try {
        ...
        try {
          ...
          if (postResponse.ok) {
            ...
            try {
              const xmlResponse = await fetch('https://api.mspace.co.ke/smsapi/v2/balance', {
                method: 'POST',
                headers: {
                  'apikey': apiKey,
              ...
              console.log('XML POST response status:', xmlResponse.status, xmlResponse.statusText);
              ...
              if (xmlResponse.ok) {
                ...
                if (xmlResponseText.includes('<balance>')) {
                  const balanceMatch = xmlResponseText.match(/<balance>(.*?)<\/balance>/);
            ...
            console.log(`POST request failed with status ${postResponse.status}: ${postResponse.statusText}`);
        ...
        try {
          ...
          console.log('GET Balance response status (format 1):', getResponse1.status);
          ...
          if (getResponse1.ok) {
            ...
            console.log(`GET request (format 1) failed with status ${getResponse1.status}: ${getResponse1.statusText}`);
            // Try alternative GET format
          }
        } catch (getError1) {
          console.log('GET method (format 1) failed, trying alternative format:', getError1);
          // Try alternative GET format
        }
        ...
        try {
          ...
          console.log('GET Balance response status (format 2):', getResponse2.status);
          ...
          if (getResponse2.ok) {
            ...
            console.log(`GET request (format 2) failed with status ${getResponse2.status}: ${getResponse2.statusText}`);
            
            // If this is the last attempt, throw an error
            if (attempt === maxRetries - 1) {
              throw new Error(`All request methods failed after ${maxRetries} attempts`);
            }
          ...
          // If this is the last attempt, throw an error
          if (attempt === maxRetries - 1) {
            throw new Error(`All request methods failed after ${maxRetries} attempts: ${getError2.message}`);
          }
        ...
        // If this is the last attempt, throw an error
        if (attempt === maxRetries - 1) {
          throw new Error(`Balance check failed after ${maxRetries} attempts: ${attemptError.message}`);
        }
