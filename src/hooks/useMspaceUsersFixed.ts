...
export const useMspaceUsersFixed = () => {
  ...
  const {
    ...
    queryFn: async (): Promise<MspaceUser[]> => {
      try {
        ...
        // Get credentials and determine if we should use direct API or edge functions
        ...
        // Now we can use the credentials directly (whether encrypted or plain text)
        ...
        try {
          ...
          if (Array.isArray(data)) {
            ...
            if (arrayProps.length > 0) {
              ...
              resellerClients = arrayProps[0].value as any[];
            }
          }
          ...
          if (response.ok) {
            ...
            if (Array.isArray(data)) {
              ...
              if (arrayProps.length > 0) {
                ...
                subAccounts = arrayProps[0].value as any[];
              }
            }
          } else {
            console.warn(
  ...
  const updateUserBalance = useMutation({
    mutationFn: async ({
      ...
      // Use direct API call (works with both encrypted and plain text credentials)
