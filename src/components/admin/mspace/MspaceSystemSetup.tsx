...
export const MspaceSystemSetup: React.FC = () => {
  ...
  const saveCredentialsMutation = useMutation({
    mutationFn: async (credentials: { username: string; apiKey: string }) => {
      ...
      // Store API key directly (unencrypted) in api_key_encrypted column
  ...
  const startConfiguring = () => {
    ...
    if (userCredentials) {
      setFormData({
        username: typeof (userCredentials as any)?.username === 'string' ? (userCredentials as any).username : '',
              ...
              <div>
                ...
                {isConfigured && (
                  <p className="text-sm text-muted-foreground">
                    Username: {typeof (userCredentials as any)?.username === 'string' ? (userCredentials as any).username : '[not set]'}
                  </p>
                )}
              </div>
            </div>
